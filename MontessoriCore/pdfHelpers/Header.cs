...
namespace Montessori.Core
{
    public class Header
    {
        ...
        public static void RightHeader(Document doc, FileStream fs, PdfWriter writer, PdfContentByte contentByte, string printDate, string examName)
        {
            ...
            ct.Go();
            /*
            contentByte.Rectangle(437, 68, doc.PageSize.Width / 4, 183);
            contentByte.Stroke();
            
            ct = new ColumnText(contentByte);
            phrase = new Phrase("Grade Point Average (GPA):_________", rf.GPAFont);
            ct.SetSimpleColumn(phrase, 440, 30, 650, 220, 15, Element.ALIGN_LEFT);
            ct.Go();

            
            ct = new ColumnText(contentByte);
            phrase = new Phrase("Average Grade:_________", rf.GPAFont);
            ct.SetSimpleColumn(phrase, 480, 30, 650, 185, 15, Element.ALIGN_LEFT);
            ct.Go();
           ...
           cell = new PdfPCell();
           cell.AddElement(new Phrase("Letter Grade", rf.GradingSystem));
           cell.HorizontalAlignment = Element.ALIGN_CENTER;
           gradeTable.AddCell(cell);
           ...
           for (int i = 0; i < grades.Count; i++)
           {
               ...
               cell = new PdfPCell();
               cell.AddElement(new Phrase(grades[i].GradePoint.ToString(), rf.GradingSystem));
               cell.HorizontalAlignment = Element.ALIGN_CENTER;
               gradeTable.AddCell(cell);
           ...
           gradeTable.WriteSelectedRows(0, -1, 652, 247, contentByte);
           */
            ct = new ColumnText(contentByte);
            phrase = new Phrase("_____________" + Environment.NewLine + " Class Teacher", rf.Arial);
            ct.SetSimpleColumn(phrase, 440, 30, 850, 60, 15, Element.ALIGN_LEFT);
            ct.Go();

            ct = new ColumnText(contentByte);
            phrase = new Phrase("______________" + Environment.NewLine + "     School Seal", rf.Arial);
        ...
        public static void LeftHeader(Document doc, FileStream fs, PdfWriter writer, PdfContentByte contentByte, string printDate, string examName)
        {
            ...
            /*
            contentByte.Rectangle(15, 68, doc.PageSize.Width / 4, 183);
            contentByte.Stroke();

            ct = new ColumnText(contentByte);
            phrase = new Phrase("Grade Point Average (GPA):_________", rf.GPAFont);
            ct.SetSimpleColumn(phrase, 22, 220, 450, 30, 15, Element.ALIGN_LEFT);
            ct.Go();

            ct = new ColumnText(contentByte);
            phrase = new Phrase("Average Grade:_________", rf.GPAFont);
            ct.SetSimpleColumn(phrase, 70, 185, 450, 30, 15, Element.ALIGN_LEFT);
            ct.Go();
           ...
           // cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            ...
            gradeTable.WriteSelectedRows(0, -1, doc.PageSize.Width / 4 + 20, 247, contentByte);
            */
            ct = new ColumnText(contentByte);
            phrase = new Phrase("_____________" + Environment.NewLine + " Class Teacher", rf.Arial);
