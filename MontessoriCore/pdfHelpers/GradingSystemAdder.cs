...
namespace Montessori.Core
{
    public class GradingSystemAdder
    {
        ...
        static string gpa = Environment.NewLine + Environment.NewLine + "Grade Point Average (GPA):________" + Environment.NewLine + Environment.NewLine + "                   Average Grade:_________";
        ...
        public static PdfPTable GradingTable(float width)
        {
            ...
            gradeTable.AddCell(cell);
            for (int i = 0; i < grades.Count - 3; i++)
            {
                cell = new PdfPCell();
